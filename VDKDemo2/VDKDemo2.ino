...
// define variables //
// Initialize variables for Auto Mode
Servo servo;
...
bool systemActive = true; // Biến kiểm tra trạng thái hoạt động của hệ thống
bool isShooting = true; //Biến kiểm tra súng bắn (DC chạy)
  ...
  if (Firebase.ready() && (millis() - currentMillis > 500 || currentMillis == 0))
    {
      ...
      if (Firebase.Firestore.getDocument(&fbdo, FIREBASE_PROJECT_ID, "", documentPath.c_str(), mask.c_str()))
      {
        ...
        bool isAutoFB = jsonData.boolValue;
        // get another variables for Auto mode
        payload.get(jsonData, "fields/isOff/booleanValue",true);
        bool systemActiveFB = jsonData.boolValue;
        payload.get(jsonData, "fields/isStop/booleanValue",true);
        bool movingServoFB = jsonData.boolValue;
        payload.get(jsonData, "fields/isIncreasing/booleanValue",true);
        bool isIncreasingFB = jsonData.boolValue;
        payload.get(jsonData, "fields/angle/integerValue",true);
        int servoAngleFB = jsonData.intValue;
        payload.get(jsonData, "fields/isShooting/booleanValue", true);
        bool isShootingFB = jsonData.boolValue;
        if (!isAuto) {
          // Auto mode off , you have to control by app 
          Running(systemActiveFB, movingServoFB, isIncreasingFB,servoAngleFB, isShootingFB); 
        } else {
          Running(systemActive,movingServo,increasingAngle,servoAngle,isShooting);
      }
    else
...
void Running(bool systemActive, bool movingServo, bool increasingAngle, int servoAngle, bool isShooting) {
// Auto mode activate
  if (systemActive) {
    ...
    if (pirState == LOW) {
      ...
      movingServo = false;             // Dừng di chuyển servo
      if (isShooting) { // Nếu isShooting = true thì bắn , false thì không bắn
        TurnOnDC();
      } else {
        TurnOffDC();
      }
